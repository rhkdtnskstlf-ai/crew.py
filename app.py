...
with st.expander("✅ 업데이트 내역 및 사용 가이드 (클릭하여 확인)", expanded=True):
    ...
    * **베이징 레이오버 처리**: 특정 출발편 번호를 등록하면 해당 크루의 체류를 3박으로 자동 처리하고, D+1~D+2 중복 체크인 행을 제거합니다. D+3 이후 별개 일정은 보존됩니다.
...
st.markdown("해당 편으로 **출발하는 크루**는 Checkin 기준 **3박으로 고정**되며, D+1~D+2 중복 체크인 행이 제거됩니다. D+3 이후 별개 일정은 그대로 유지됩니다.")
...
if uploaded_file:
    ...
    # ==============================
    # 베이징 레이오버 처리
    # 1단계: 레이오버 출발편으로 나가는 크루 파악 + 출발일 기록
    # ==============================
    extended_crew_ids = set()
    layover_dep_date_by_crew = {}  # crew_id -> 레이오버 Dep Flt Date (date)
    layover_flt_by_crew = {}       # crew_id -> 레이오버 출발편 번호 문자열

    if layover_flights:
        dep_flt_upper = df['Dep Flt'].astype(str).str.strip().str.upper()
        layover_mask = dep_flt_upper.isin(layover_flights)
        layover_rows = df[layover_mask]

        for _, row in layover_rows.iterrows():
            crew_id = row['Crew ID']
            dep_date = row['Dep Flt Date'].date()
            dep_flt = str(row['Dep Flt']).strip()
            extended_crew_ids.add(crew_id)

            if crew_id not in layover_dep_date_by_crew or dep_date < layover_dep_date_by_crew[crew_id]:
                layover_dep_date_by_crew[crew_id] = dep_date
                layover_flt_by_crew[crew_id] = dep_flt

        # 2단계: 레이오버 출발일 D+1, D+2 체크인 행만 제거
        # D+3 이후 별개 일정은 보존
        def should_drop(row):
            crew_id = row['Crew ID']
            if crew_id not in layover_dep_date_by_crew:
                return False
            dep_date = layover_dep_date_by_crew[crew_id]
            arr_date = row['Arr Flt Date'].date()
            days_after = (arr_date - dep_date).days
            return 1 <= days_after <= 2
    ...
    # ==============================
    # stay 집계
    # Dep_Flt: 레이오버 편수 포함된 stay면 그 편수 우선 사용 (기존 last() 대체)
    # ==============================
    stay_records = []
    for stay_id, group in df.groupby('stay_id'):
        ...
        # Dep_Flt 결정: 레이오버 편수 우선, 없으면 마지막 편수
    ...
    # ==============================
    # 레이오버 기장: Checkout을 Checkin+3일로 고정 (=3박)
    # ==============================
    if layover_dep_date_by_crew:
        for idx, row in stay_df.iterrows():
            crew_id = row['Crew ID']
            if crew_id not in layover_dep_date_by_crew:
                continue
            dep_date     = layover_dep_date_by_crew[crew_id]
            checkin_date = row['Checkin'].date()
            # 레이오버 출발일을 포함하는 stay에만 적용
            if checkin_date <= dep_date:
    ...
    if extended_crew_ids:
        processed_names = stay_df[stay_df['Crew ID'].isin(extended_crew_ids)]['Crew Name'].unique()
        st.info(f"🔄 레이오버 처리 적용됨: **{', '.join(processed_names)}** ({len(extended_crew_ids)}명) — 3박 고정, D+1~D+2 중복 행 제거 완료")
    ...
    for _, row in stay_df.iterrows():
        ...
        # 레이오버 기장: 하늘색 / 중복 Crew ID: 노란색 (레이오버 우선)
        if row['Crew ID'] in extended_crew_ids:
            for col in range(1, 9):
